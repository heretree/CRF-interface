...
def clean_and_tokenize(sentence):
    '''
        去除多余空白、分词、清洗
    '''
    sentence_list = [char for char in sentence if unicodedata.category(char) != 'Mn']
    sentence = ''.join(sentence_list)
    sentence = re.sub(r'\s+', ' ', sentence)
    sentence = sentence.strip()
    token_list = word_tokenize(sentence)
    token_list = to_lower(token_list)
    # token_list = delete_characters(token_list)
    return token_list
...
def stem(token):
    ...
    return word_lematizer

def stem_words(token_words):
    '''
        词形归一化：list of tokens
    '''
    words_lematizer = []
    for token in token_words:
        words_lematizer.append(stem(token))
    return words_lematizer

def normalize(text):
    word_list = text.split()
    word_list = [stem((w, 'NN')) for w in word_list]
    word_list = [stem((w, 'VB')) for w in word_list]
...
class UnicodeBlock(object):
    def __init__(self):
        ...
        for line in block_file.splitlines():
            m = pattern.match(line)
            if m:
                start, end, name = m.groups()
                self._blocks.append((int(start, 16), int(end, 16), name))
        return

    def unicode_block(self, char):
        '''
        Return the Unicode block name for char, or None if char has no block.
        ...
        cp = ord(char)
        for start, end, name in self._blocks:
            if start <= cp <= end:
